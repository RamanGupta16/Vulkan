...
const std::vector<Vertex> vertices = {
    {{-0.5f, -0.5f}, {1.0f, 0.0f, 0.0f}}, // Top Left is red
    {{0.5f, -0.5f}, {0.0f, 1.0f, 0.0f}}, // top-right is green
    {{0.5f, 0.5f}, {0.0f, 0.0f, 1.0f}}, // bottom-right is blue
    {{-0.5f, 0.5f}, {1.0f, 1.0f, 1.0f}} // bottom-left is white
};
...
private:
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex /*index of the current swapchain image we want to write to*/) {
        ...
        // Using an index buffer https://vulkan-tutorial.com/en/Vertex_buffers/Index_buffer
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
        ...
        std::cout << "CreateBuffer allocationSize=" << memRequirements.size << std::endl;
    ...
    // Index buffer creation https://vulkan-tutorial.com/en/Vertex_buffers/Index_buffer
    void createIndexBuffer() {
        
        VkDeviceSize bufferSize = sizeof(indices[0]) * indices.size();
        
        // Using a staging buffer https://vulkan-tutorial.com/en/Vertex_buffers/Index_buffer
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingBufferMemory;
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            stagingBuffer, stagingBufferMemory); // VK_BUFFER_USAGE_TRANSFER_SRC_BIT: Buffer can be used as source in a memory transfer operation.
        ...
        // Create device local indexBuffer
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMemory);

        // We copy data from the indices data from stagingBuffer to device local indexBuffer
